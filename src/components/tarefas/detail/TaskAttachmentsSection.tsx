...
import {
  useTarefaAttachments,
  useUploadTarefaAttachment,
  useDeleteTarefaAttachment,
  useTarefaAttachmentSignedUrl,
  TarefaAttachment,
} from "@/hooks/useTarefaAttachments";
import { UploadFileModal } from "@/components/wiki/UploadFileModal";
import { Paperclip, Plus, FileText, Image, File, Trash2, Download, Loader2, Eye } from "lucide-react";
...
function AttachmentRow({
  ...
  attachment: TarefaAttachment;
  onDelete: () => void;
}) {
  const { data: signedUrl, isLoading: loadingUrl } = useTarefaAttachmentSignedUrl(attachment.storage_path);
...
export function TaskAttachmentsSection({ tarefaId }: TaskAttachmentsSectionProps) {
  const [uploadOpen, setUploadOpen] = useState(false);
  const { data: attachments = [], isLoading } = useTarefaAttachments(tarefaId);
  const uploadAttachment = useUploadTarefaAttachment();
  const deleteAttachment = useDeleteTarefaAttachment();
  ...
  const handleDelete = (a: TarefaAttachment) => {
