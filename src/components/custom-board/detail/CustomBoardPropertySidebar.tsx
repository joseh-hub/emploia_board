...
import type { CustomBoardColumn } from "@/hooks/useCustomBoardColumns";
...
export interface CustomBoardPropertySidebarProps {
  boardId: string;
  cardId: string;
...
export function CustomBoardPropertySidebar({
  boardId,
  cardId,
  columnId,
  columns,
  ...
  profiles,
  isMovingColumn,
}: CustomBoardPropertySidebarProps) {
    ...
    <div className="space-y-1">
      ...
      <PropertyRow icon={Columns3} label="Status">
        <Select
          ...
          disabled={isMovingColumn || columns.length === 0}
        >
          <SelectTrigger className="h-8 border-0 bg-transparent hover:bg-zinc-800/50 focus:ring-0 justify-start gap-2 px-2 -mx-2 text-zinc-200">
            <SelectValue>
              {currentColumn ? (
                ...
                <span className="text-zinc-500">Selecionar coluna</span>
              )}
            </SelectValue>
          </SelectTrigger>
      ...
      <PropertyRow icon={Users} label="Responsáveis">
        <Popover open={assigneesOpen} onOpenChange={setAssigneesOpen}>
              ...
              {assignedProfiles.length > 0 ? (
                <div className="flex items-center gap-1 flex-wrap">
                  ...
                  {assignedProfiles.length > 3 && (
                    <span className="text-xs text-zinc-400 ml-1">+{assignedProfiles.length - 3}</span>
                  )}
          ...
          <PopoverContent className="w-80 p-3 bg-zinc-900 border-zinc-800" align="start">
            <UserMultiSelect
              selectedUsers={assignedUsers.filter((u) => profiles.some((p) => p.id === u))}
